...
app = Flask(__name__)
app.secret_key = "hallowbu"  # Replace with your own secret key, needed for session management
...
def generate_data(N, mu, beta0, beta1, sigma2, S):
    ...
    plt.plot(X, model.predict(X.reshape(-1, 1)), color="r", label="Regression Line")
    plt.xlabel("X")
    plt.ylabel("Y")
    ...
    plt.axvline(slope, color="r", linestyle="--", label="Observed Slope")
    ...
    slope_more_extreme = sum(abs(np.array(slopes) - beta1) >= abs(slope - beta1)) / S  # Replace with code to calculate proportion of slopes more extreme than observed
    intercept_extreme = sum(abs(np.array(intercepts) - beta0) >= abs(intercept - beta0)) / S  # Replace with code to calculate proportion of intercepts more extreme than observed
...
def hypothesis_test():
    ...
    plt.axvline(observed_stat, color="r", linestyle="--", label="Observed Value")
    plt.axvline(hypothesized_value, color="g", linestyle="--", label="Hypothesized Value")
...
def confidence_interval():
    ...
    margin_of_error = 1.96 * (std_estimate / np.sqrt(S))
    ...
    plt.figure()
    plt.scatter(estimates, range(len(estimates)), alpha=0.5, label="Estimates")
    plt.axvline(mean_estimate, color="r", linestyle="--", label="Mean Estimate")
    plt.axvline(ci_lower, color="g", linestyle="--", label="CI Lower Bound")
    plt.axvline(ci_upper, color="g", linestyle="--", label="CI Upper Bound")
    plt.axvline(true_param, color="b", linestyle="--", label="True Parameter")
    plt.title("Confidence Interval Visualization (Vertical)")
    plt.xlabel("Value")
    plt.ylabel("Index")
    plt.legend()
